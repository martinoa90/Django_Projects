...
class Index(View):
    ...
    def get(self, request):
        if 'user' in request.session:
            get_save_pwd = SavePassword.objects.filter(
                user__name=request.session['user']).order_by('-id')
...
class SavePwd(View):
    ...
    def post(self, request):
        ...
        savePwd = SavePassword.objects.create(
            user=user_obj, title=title, pwd=pwd, hide_pwd=makepwd)
        savePwd.save()
...
class Edit(View):
    ...
    def post(self, request):
        ...
        new_pdw_hash = make_password(new_pwd)

        pwd_obj.title = new_title
        pwd_obj.pwd = new_pwd
        pwd_obj.hide_pwd = new_pdw_hash
